...
IndexBuffer::~IndexBuffer() {
    BE_GL_LOG("GL: glDeleteBuffers(1, {})", m_IndexBuffer);
    glDeleteBuffers(1, &m_IndexBuffer);
}

void IndexBuffer::Bind() const {
    BE_GL_LOG("GL: glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, {})", m_IndexBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IndexBuffer);
}

void IndexBuffer::Unbind() const {
    BE_GL_LOG("GL: glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0)");
...
void IndexBuffer::SetData(const void* data, u32 size) {
    ...
    BE_GL_LOG("GL: glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, {}, {})", size,
              data);
...
IndexBuffer::IndexBuffer(const void* data, u32 size) : m_Size(size) {
    ...
    BE_GL_LOG("GL: glGenBuffers(1, {})", m_IndexBuffer);

    Bind();

    glBufferData(GL_ELEMENT_ARRAY_BUFFER, size, data,
                 data == nullptr ? GL_DYNAMIC_DRAW : GL_STATIC_DRAW);
    BE_GL_LOG("GL: glBufferData(GL_ELEMENT_ARRAY_BUFFER, {}, {}, {})", size,
              data, data == nullptr ? "GL_DYNAMIC_DRAW" : "GL_STATIC_DRAW");
}
}  // namespace BillyEngine